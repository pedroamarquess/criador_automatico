...
class GratRepApp():
    ...
    def __init__(self):
        self.janela = tk.Tk()
        self.janela.config(background="white", padx=20, pady=20)
        self.janela.geometry("900x500")

        #carrega a planilha base do excel
        self.wb = load_workbook(self.caminho_absoluto("novo_arquivo_base.xlsx"))
        self.wb_qsgr = self.wb["QSGR"]
        self.ws_info = self.wb["INFO"]

        #inseri o nome e graduacao dos militares no excel
        self.inserir_nomes()
    ...
    def inserir_nomes(self):
        numero_celula = 16
        arquivo_csv_nomes = self.caminho_absoluto(separar_nomes())
        df = pd.read_csv(arquivo_csv_nomes)
        lista_graduacao = df["graduacao"].to_list()
        lista_nome = df["nome_completo"].to_list()
        for i in range(len(lista_nome)):
            self.wb_qsgr[f"B{numero_celula}"] = lista_graduacao[i]
            self.wb_qsgr[f"D{numero_celula}"] = lista_nome[i]
            numero_celula  += 1
    ...
    def refazer_tela(self):
        for widget in self.widgets:
            widget.grid_forget()
        botao_refazer = tk.Button(text="Fazer outra grat_rep", command=self.nova_solicitacao, bg="white", fg="black")
        botao_refazer.pack(expand=True)
        self.widgets = [botao_refazer]
    ...
    def caminho_absoluto(self,relativo):
        """
